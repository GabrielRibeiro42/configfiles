...
var init_main = __esm({
  "node_modules/vscode-languageserver-types/lib/esm/main.js"() {
    ...
    EOL = ["\n", "\r\n", "\r"];
    (function(TextDocument16) {
      function create(uri, languageId, version, content) {
        return new FullTextDocument(uri, languageId, version, content);
      }
      TextDocument16.create = create;
      function is(value) {
        var candidate = value;
        return Is.defined(candidate) && Is.string(candidate.uri) && (Is.undefined(candidate.languageId) || Is.string(candidate.languageId)) && Is.uinteger(candidate.lineCount) && Is.func(candidate.getText) && Is.func(candidate.positionAt) && Is.func(candidate.offsetAt) ? true : false;
      }
      TextDocument16.is = is;
      ...
      TextDocument16.applyEdits = applyEdits;
...
function getInlayHintsPreferences(language2) {
  const config = import_coc.workspace.getConfiguration(language2);
  return {
...
var _WatchProject = class {
  constructor(client) {
    ...
    this.disposables.push(import_coc2.commands.registerCommand(_WatchProject.id, async () => {
...
var TypeScriptWorkspaceSymbolProvider = class {
  ...
  async provideWorkspaceSymbols(search, token) {
    ...
    for (const item of response.body) {
      ...
      const symbolInfo = import_vscode_languageserver_protocol2.SymbolInformation.create(label, getSymbolKind(item), range, this.client.toResource(item.file));
  ...
  get searchAllOpenProjects() {
    return this.client.apiVersion.gte(API.v390) && import_coc3.workspace.getConfiguration("typescript").get("workspaceSymbols.scope", "allOpenProjects") === "allOpenProjects";
  }
};
...
function convertCompletionEntry(tsEntry, uri, position, context) {
  ...
  if (tsEntry.kindModifiers) {
    ...
    if (kindModifiers.has(KindModifiers.deprecated)) {
      tags = [import_vscode_languageserver_protocol6.CompletionItemTag.Deprecated];
    }
...
var TypeScriptInlayHintsProvider = class {
  constructor(client, fileConfigurationManager, languageIds) {
    ...
    this.fileConfigurationManager = fileConfigurationManager;
    this.languageIds = languageIds;
    this.inlayHintsNS = import_coc14.workspace.createNameSpace("tsserver-inlay-hint");
    this._disposables = [];
    ...
    import_coc14.workspace.onDidChangeConfiguration(async (e) => {
    ...
    import_coc14.workspace.onDidOpenTextDocument(async (e) => {
      const doc = import_coc14.workspace.getDocument(e.bufnr);
      await this.syncAndRenderHints(doc);
    }, null, this._disposables);
    import_coc14.workspace.onDidChangeTextDocument(async (e) => {
      const doc = import_coc14.workspace.getDocument(e.bufnr);
  ...
  dispose() {
    ...
    (0, import_coc14.disposeAll)(this._disposables);
    this._disposables = [];
  ...
  async syncAndRenderHints(doc) {
    ...
    try {
      this._tokenSource = new import_coc14.CancellationTokenSource();
      const { token } = this._tokenSource;
      const range = import_coc14.Range.create(0, 0, doc.lineCount, doc.getline(doc.lineCount).length);
  ...
  async provideInlayHints(document, range, token) {
    ...
    return response.body.map((hint) => {
      return {
        text: hint.text,
        position: import_coc14.Position.create(hint.position.line - 1, hint.position.offset - 1),
...
async function getManager() {
  let res = await import_coc15.workspace.findUp(["yarn.lock", "package-lock.json"]);
  if (!res)
    return "yarn";
...
async function installModules(uri, names) {
  ...
  let workspaceFolder = import_coc15.workspace.getWorkspaceFolder(uri);
  let root = workspaceFolder ? import_coc15.Uri.parse(workspaceFolder.uri).fsPath : void 0;
  if (!root || !import_fs.default.existsSync(import_path3.default.join(root, "package.json"))) {
    import_coc15.window.showMessage(`package.json not found from workspaceFolder: ${root}`, "error");
    return;
  }
  let arr = names.concat(names.map((s) => `@types/${s}`));
  let statusItem = import_coc15.window.createStatusBarItem(99, { progress: true });
  ...
  try {
    ...
    statusItem.dispose();
    import_coc15.window.showMessage(`Install error ${e.message}`, "error");
    return;
  }
  statusItem.dispose();
  import_coc15.window.showMessage(`Installed: ${exists.join(" ")}`, "more");
}
...
var InstallModuleProvider = class {
  constructor(client) {
    this.client = client;
    import_coc16.commands.registerCommand("_tsserver.installModule", async (uri, name) => {
      await installModules(uri, [name]);
    });
  }
  async provideCodeActions(document, _range, context, _token) {
    const uri = import_coc16.Uri.parse(document.uri);
    if (uri.scheme != "file")
      return null;
...
var ReloadProjectsCommand = class {
  ...
  async execute() {
    ...
    import_coc17.window.showMessage("projects reloaded");
  }
};
...
async function goToProjectConfig(clientHost, uri) {
  ...
  let res;
  try {
    res = await client.execute("projectInfo", { file: file2, needFileNameList: false }, import_coc17.CancellationToken.None);
  } catch {
  }
  if (!res || !res.body) {
    import_coc17.window.showMessage("Could not determine TypeScript or JavaScript project.", "warning");
    return;
  }
  const { configFileName } = res.body;
  if (configFileName && !isImplicitProjectConfigFile(configFileName)) {
    await import_coc17.workspace.openResource(import_coc17.Uri.file(configFileName).toString());
    return;
  }
  import_coc17.window.showMessage("Config file not found", "warning");
}
...
var AutoFixCommand = class {
  ...
  async execute() {
    if (this.service.state != import_coc17.ServiceStat.Running) {
      throw new Error("service not running");
      return;
    }
    let client = await this.service.getClientHost();
    let document = await import_coc17.workspace.document;
    ...
    for (let diagnostic of diagnostics) {
      ...
      const response = await client.serviceClient.execute("getCodeFixes", args, import_coc17.CancellationToken.None);
...
`
    ...
    if (command)
      import_coc17.commands.executeCommand(command);
  }
};
...
var _FileReferencesCommand = class {
  ...
  async execute() {
    ...
    if (client.serviceClient.apiVersion.lt(_FileReferencesCommand.minVersion)) {
      import_coc17.window.showMessage("Find file references failed. Requires TypeScript 4.2+.", "error");
      return;
    }
    ...
    if (!openedFiledPath)
      return;
    const response = await client.serviceClient.execute("fileReferences", { file: openedFiledPath }, import_coc17.CancellationToken.None);
    if (response.type !== "response" || !response.body)
      return;
    const locations = response.body.refs.map((r) => Location2.fromTextSpan(client.serviceClient.toResource(r.file), r));
    await import_coc17.commands.executeCommand("editor.action.showReferences", doc.uri, Position.create(0, 0), locations);
  }
};
...
function registCommand(cmd) {
  let { id, execute } = cmd;
  return import_coc17.commands.registerCommand(id, execute, cmd);
}
...
var _ApplyRefactoringCommand = class {
  ...
  async execute(document, file2, refactor, action, range) {
    ...
    if (!await import_coc19.workspace.applyEdit(workspaceEdit)) {
      return false;
    }
    const renameLocation = body.renameLocation;
    if (renameLocation) {
      import_coc19.commands.executeCommand("editor.action.rename", document.uri, Position2.fromLocation(renameLocation));
    }
    return true;
  }
  async toWorkspaceEdit(body) {
    for (const edit of body.edits) {
      await import_coc19.workspace.createFile(edit.fileName, { ignoreIfExists: true });
    }
...
var _SelectRefactorCommand = class {
  ...
  async execute(document, file2, info, range) {
    let { actions } = info;
    const idx = actions.length == 1 ? 0 : await import_coc19.window.showQuickpick(actions.map((action) => action.description || action.name));
    if (idx == -1)
      return false;
...
var TypeScriptRenameProvider = class {
  ...
  async renameFile(fileToRename, newName, token) {
    ...
    edits.documentChanges.push({
      kind: "rename",
      oldUri: import_coc20.Uri.file(fileToRename).toString(),
      newUri: import_coc20.Uri.file(newFilePath).toString(),
      options: {
        overwrite: false,
        ignoreIfExists: true
      }
...
var TypeScriptDocumentSemanticTokensProvider = class {
  ...
  async _provideSemanticTokens(document, requestArg, token) {
    ...
    const builder = new import_coc21.SemanticTokensBuilder();
    let i = 0;
...
var _TagClosing = class {
  constructor(client, descriptionLanguageId) {
    ...
    this._cancel = void 0;
    import_coc22.events.on("InsertCharPre", (character) => {
      this.lastInsert = character;
    }, null, this._disposables);
    import_coc22.events.on("TextChangedI", this.onChange, this, this._disposables);
    import_coc22.events.on("TextChangedP", this.onChange, this, this._disposables);
  }
  async onChange(bufnr, change) {
    let doc = import_coc22.workspace.getDocument(bufnr);
    if (!doc || !doc.attached)
      return;
    ...
    this._timeout = setTimeout(async () => {
      ...
      if (doc.changedtick > changedtick)
        return;
      const position = import_coc22.Position.create(lnum - 1, pre.length);
      const args = Position2.toFileLocationRequestArgs(filepath, position);
      this._cancel = new import_coc22.CancellationTokenSource();
      ...
      if (doc.changedtick === changedtick) {
        import_coc22.snippetManager.insertSnippet(this.getTagSnippet(insertion).value, false, import_coc22.Range.create(position, position));
      }
    }, 50);
  }
  isEnabled(languageId, uri) {
    ...
    if (!import_coc22.workspace.getConfiguration(void 0, uri).get(`${configLang}.autoClosingTags`)) {
      return false;
    }
    return true;
  }
  dispose() {
    ...
    (0, import_coc22.disposeAll)(this._disposables);
    this._disposables = [];
  }
...
TagClosing.minVersion = API.v300;
...
var UpdateImportsOnFileRenameHandler = class {
  constructor(client, fileConfigurationManager, languageId) {
    ...
    const watcher = import_coc23.workspace.createFileSystemWatcher(glob);
  ...
  dispose() {
    (0, import_coc23.disposeAll)(this.disposables);
  }
  async doRename(oldResource, newResource) {
    ...
    let oldDocument = import_coc23.workspace.getDocument(oldResource.toString());
    if (oldDocument) {
      await import_coc23.workspace.nvim.command(`silent ${oldDocument.bufnr}bwipeout!`);
    }
    let document = import_coc23.workspace.getDocument(newUri);
    if (document) {
      await import_coc23.workspace.nvim.command(`silent ${document.bufnr}bwipeout!`);
      await wait(30);
    }
    document = await import_coc23.workspace.loadFile(newUri);
    if (!document)
      return;
    await wait(50);
    ...
    if (await this.promptUser(newResource)) {
      await import_coc23.workspace.applyEdit(edits);
    }
  }
  async promptUser(newResource) {
    return await import_coc23.window.showPrompt(`Update imports for moved file: ${newResource.fsPath}?`);
  }
...
var OrganizeImportsCommand = class {
  ...
  async _execute(client, document, sortOnly = false) {
    ...
    if (keys.length == 1) {
      let doc = import_coc24.workspace.getDocument(keys[0]);
      if (doc) {
    ...
    if (edit)
      await import_coc24.workspace.applyEdit(edit);
  }
  async execute(document, sortOnly = false) {
    let client = await this.service.getClientHost();
    if (!document) {
      let doc = await import_coc24.workspace.document;
      if (!doc.attached) {
...
var OrganizeImportsCodeActionProvider = class {
  ...
  async provideCodeActions(document, _range, context, token) {
    ...
    const organizeImportsAction = import_vscode_languageserver_protocol16.CodeAction.create("Organize Imports", {
      title: "",
      command: "tsserver.organizeImports",
      arguments: [document]
    }, import_vscode_languageserver_protocol16.CodeActionKind.SourceOrganizeImports);
    const sortImportsAction = import_vscode_languageserver_protocol16.CodeAction.create("Sort Imports", {
      title: "",
...
var LanguageProvider = class {
  constructor(client, fileConfigurationManager, description, typingsStatus) {
    ...
    this.disposables = [];
    import_coc25.workspace.onDidChangeConfiguration(this.configurationChanged, this, this.disposables);
  ...
  configurationChanged() {
    const config = import_coc25.workspace.getConfiguration(this.id, null);
  ...
  registerProviders(client, typingsStatus) {
    ...
    this._register(import_coc25.languages.registerCompletionItemProvider(clientId, "TSC", languageIds, new TypeScriptCompletionItemProvider(client, typingsStatus, this.fileConfigurationManager, this.description.id), TypeScriptCompletionItemProvider.triggerCharacters));
    if (this.client.apiVersion.gte(API.v230)) {
      this._register(import_coc25.languages.registerCompletionItemProvider(`${this.description.id}-directive`, "TSC", languageIds, new DirectiveCommentCompletionProvider(client), ["@"]));
    }
    let definitionProvider = new TypeScriptDefinitionProvider(client);
    this._register(import_coc25.languages.registerDefinitionProvider(languageIds, definitionProvider));
    this._register(import_coc25.languages.registerTypeDefinitionProvider(languageIds, definitionProvider));
    this._register(import_coc25.languages.registerImplementationProvider(languageIds, definitionProvider));
    this._register(import_coc25.languages.registerReferencesProvider(languageIds, new TypeScriptReferences(client)));
    this._register(import_coc25.languages.registerHoverProvider(languageIds, new TypeScriptHoverProvider(client)));
    this._register(import_coc25.languages.registerDocumentHighlightProvider(languageIds, new TypeScriptDocumentHighlightProvider(this.client)));
    this._register(import_coc25.languages.registerSignatureHelpProvider(languageIds, new TypeScriptSignatureHelpProvider(client), ["(", ",", "<", ")"]));
    this._register(import_coc25.languages.registerDocumentSymbolProvider(languageIds, new TypeScriptDocumentSymbolProvider(client)));
    this._register(import_coc25.languages.registerRenameProvider(languageIds, new TypeScriptRenameProvider(client, this.fileConfigurationManager)));
    let formatProvider = new TypeScriptFormattingProvider(client, this.fileConfigurationManager);
    this._register(import_coc25.languages.registerDocumentFormatProvider(languageIds, formatProvider));
    this._register(import_coc25.languages.registerDocumentRangeFormatProvider(languageIds, formatProvider));
    this._register(import_coc25.languages.registerOnTypeFormattingEditProvider(languageIds, formatProvider, [";", "}", "\n", String.fromCharCode(27)]));
    this._register(import_coc25.languages.registerCodeActionProvider(languageIds, new InstallModuleProvider(client), "tsserver"));
    if (this.client.apiVersion.gte(API.v380) && typeof import_coc25.languages["registerCallHierarchyProvider"] === "function") {
      this._register(import_coc25.languages.registerCallHierarchyProvider(languageIds, new TypeScriptCallHierarchySupport(client)));
    }
    if (this.client.apiVersion.gte(API.v370)) {
      const provider = new TypeScriptDocumentSemanticTokensProvider(client);
      if (typeof import_coc25.languages["registerDocumentSemanticTokensProvider"] === "function") {
        this._register(import_coc25.languages.registerDocumentSemanticTokensProvider(languageIds, provider, provider.getLegend()));
      }
      if (typeof import_coc25.languages["registerDocumentRangeSemanticTokensProvider"] === "function") {
        this._register(import_coc25.languages.registerDocumentRangeSemanticTokensProvider(languageIds, provider, provider.getLegend()));
      }
    }
    ...
    if (this.client.apiVersion.gte(API.v280)) {
      this._register(import_coc25.languages.registerFoldingRangeProvider(languageIds, new TypeScriptFoldingProvider(this.client)));
      this._register(import_coc25.languages.registerCodeActionProvider(languageIds, new OrganizeImportsCodeActionProvider(this.client, this.fileConfigurationManager), "tsserver", [import_vscode_languageserver_protocol17.CodeActionKind.SourceOrganizeImports]));
    }
    if (this.client.apiVersion.gte(API.v240)) {
      this._register(import_coc25.languages.registerCodeActionProvider(languageIds, new TypeScriptRefactorProvider(client, this.fileConfigurationManager), "tsserver", [import_vscode_languageserver_protocol17.CodeActionKind.Refactor]));
    }
    this._register(import_coc25.languages.registerCodeActionProvider(languageIds, new TypeScriptQuickFixProvider(client, this.fileConfigurationManager), "tsserver", [import_vscode_languageserver_protocol17.CodeActionKind.QuickFix]));
    this._register(import_coc25.languages.registerCodeActionProvider(languageIds, new ImportFixProvider(this.client.bufferSyncSupport), "tsserver", [import_vscode_languageserver_protocol17.CodeActionKind.QuickFix]));
    let cachedResponse = new CachedNavTreeResponse();
    if (this.client.apiVersion.gte(API.v206) && conf.get("referencesCodeLens.enable")) {
      this._register(import_coc25.languages.registerCodeLensProvider(languageIds, new TypeScriptReferencesCodeLensProvider(client, cachedResponse)));
    }
    if (this.client.apiVersion.gte(API.v220) && conf.get("implementationsCodeLens.enable")) {
      this._register(import_coc25.languages.registerCodeLensProvider(languageIds, new TypeScriptImplementationsCodeLensProvider(client, cachedResponse)));
    }
    if (this.client.apiVersion.gte(API.v350)) {
      this._register(import_coc25.languages.registerSelectionRangeProvider(languageIds, new SmartSelection(this.client)));
    }
    if (this.client.apiVersion.gte(API.v300)) {
      this._register(new TagClosing(this.client, this.description.id));
    }
    if (this.client.apiVersion.gte(API.v440) && import_coc25.workspace.isNvim) {
  ...
  handles(resource, doc) {
    ...
    return this.handlesConfigFile(import_coc25.Uri.parse(resource));
  }
  ...
  diagnosticsReceived(diagnosticsKind, file2, diagnostics) {
    const config = import_coc25.workspace.getConfiguration(this.id, file2.toString());
...
function defaultPathNormalizer(resource) {
  let u = import_coc26.Uri.parse(resource);
  if (u.scheme === "file") {
    return u.fsPath;
  }
...
var SyncedBufferMap = class extends ResourceMap {
  getForPath(filePath) {
    return this.get(import_coc27.Uri.file(filePath).toString());
  }
  get allBuffers() {
    return this.values;
  }
...
var BufferSyncSupport = class {
  constructor(client, modeIds) {
    ...
    import_coc27.workspace.onDidChangeConfiguration(this.updateConfiguration, this, this.disposables);
  }
  listen() {
    if (this.listening)
      return;
    this.listening = true;
    import_coc27.workspace.onDidOpenTextDocument(this.openTextDocument, this, this.disposables);
    import_coc27.workspace.onDidCloseTextDocument(this.onDidCloseTextDocument, this, this.disposables);
    import_coc27.workspace.onDidChangeTextDocument(this.onDidChangeTextDocument, this, this.disposables);
    import_coc27.workspace.textDocuments.forEach(this.openTextDocument, this);
  }
  handles(resource) {
  ...
  ensureHasBuffer(resource) {
    ...
    const existingDocument = import_coc27.workspace.textDocuments.find((doc) => doc.uri.toString() === resource);
    if (existingDocument) {
  ...
  openTextDocument(document) {
    ...
    const filepath = this.client.normalizePath(import_coc27.Uri.parse(resource));
    if (!filepath) {
      return false;
    }
    if (this.syncedBuffers.has(resource)) {
  ...
  sendPendingDiagnostics() {
    ...
    if (orderedFileSet.size) {
      let uris = Array.from(orderedFileSet.uris).map((uri) => import_coc27.Uri.parse(uri));
  ...
  updateConfiguration() {
    const jsConfig = import_coc27.workspace.getConfiguration("javascript", null);
    const tsConfig = import_coc27.workspace.getConfiguration("typescript", null);
...
var DiagnosticsManager = class {
  constructor() {
    ...
    this._currentDiagnostics = import_coc28.languages.createDiagnosticCollection("tsserver");
  }
  dispose() {
  ...
  diagnosticsReceived(kind, uri, diagnostics) {
    ...
    if (!collection)
      return;
    let doc = import_coc28.workspace.getDocument(uri);
    if (doc)
      uri = doc.uri;
    if (diagnostics.length === 0) {
...
var TypeScriptServiceConfiguration = class {
  constructor() {
    this._configuration = import_coc29.workspace.getConfiguration("tsserver");
    this._includePackageJsonAutoImports = import_coc29.workspace.getConfiguration("typescript").get("preferences.includePackageJsonAutoImports");
    import_coc29.workspace.onDidChangeConfiguration(() => {
      this._configuration = import_coc29.workspace.getConfiguration("tsserver");
      this._includePackageJsonAutoImports = import_coc29.workspace.getConfiguration("typescript").get("preferences.includePackageJsonAutoImports");
    });
  }
  ...
  get npmLocation() {
    ...
    if (path9)
      return import_coc29.workspace.expand(path9);
    try {
...
var Logger = class {
  get output() {
    ...
    this._channel = import_coc30.window.createOutputChannel("tsserver");
    return this._channel;
  }
  dispose() {
    if (this._channel) {
...
var Tracer = class {
  ...
  static readTrace() {
    let result = Trace.fromString(import_coc31.workspace.getConfiguration("tsserver").get("trace.server", "off"));
...
var TypeScriptVersionProvider = class {
  ...
  get globalVersion() {
    let { globalTsdk } = this.configuration;
    if (globalTsdk)
      return new TypeScriptVersion(import_coc32.workspace.expand(globalTsdk));
    return void 0;
  }
  getLocalVersion() {
    let folders = import_coc32.workspace.workspaceFolders.map((f) => import_coc32.Uri.parse(f.uri).fsPath);
  ...
  get bundledVersion() {
    try {
      ...
      import_coc32.window.showMessage("Bundled typescript module not found", "error");
      return null;
    }
  }
};
...
var VersionStatus = class {
  constructor(_normalizePath, enableJavascript) {
    ...
    this._versionString = "";
    this._versionBarEntry = import_coc33.window.createStatusBarItem(99);
    this._onChangeEditorSub = import_coc33.events.on("BufEnter", this.onBufEnter, this);
  ...
  async onBufEnter(bufnr) {
    let filetype = await import_coc33.workspace.nvim.call("getbufvar", [bufnr, "&filetype", ""]);
...
var TypeScriptServiceClient = class {
  ...
  restartTsServer() {
    if (this.tsServerProcess) {
      this.state = import_coc35.ServiceStat.Stopping;
  ...
  stop() {
    return new Promise((resolve) => {
      ...
      if (tsServerProcess && this.state == import_coc35.ServiceStat.Running) {
        this.info("Killing TS Server");
  ...
  startService(resendModels = false) {
    ...
    if (!currentVersion || !currentVersion.isValid) {
      if (this.configuration.globalTsdk) {
        import_coc35.window.showErrorMessage(`Can not find typescript module, in 'tsserver.tsdk': ${this.configuration.globalTsdk}`);
      } else {
        import_coc35.window.showErrorMessage(`Can not find typescript module, run ':CocInstall coc-tsserver' to fix it!`);
      }
      return;
    }
  ...
  startProcess(currentVersion, args, options, resendModels) {
    ...
    try {
      ...
      this.state = import_coc35.ServiceStat.Running;
      ...
      handle.onError((err) => {
        if (this.token != myToken)
          return;
        import_coc35.window.showErrorMessage(`TypeScript language server exited with error. Error message is: ${err.message}`);
        this.error("TSServer errored with error.", err);
        this.error(`TSServer log file: ${this.tsServerLogFile || ""}`);
        import_coc35.window.showMessage(`TSServer errored with error. ${err.message}`, "error");
      ...
      this.state = import_coc35.ServiceStat.StartFailed;
  ...
  async openTsServerLogFile() {
    ...
    let echoErr = (msg) => {
      import_coc35.window.showErrorMessage(msg);
    };
    if (isRoot) {
    ...
    try {
      await import_coc35.workspace.nvim.command(`edit ${this.tsServerLogFile}`);
      return true;
    } catch {
  ...
  serviceExited(restart) {
    this.state = import_coc35.ServiceStat.Stopped;
    this.tsServerLogFile = null;
    ...
    if (restart) {
      ...
      if (this.numberRestarts > 5) {
        ...
        if (diff < 10 * 1e3) {
          this.lastStart = Date.now();
          startService = false;
          import_coc35.window.showMessage("The TypeScript language service died 5 times right after it got started.", "error");
        } else if (diff < 60 * 1e3) {
          this.lastStart = Date.now();
          import_coc35.window.showMessage("The TypeScript language service died unexpectedly 5 times in the last 5 Minutes.", "error");
        }
      }
      if (startService) {
        this.startService(true);
  ...
  toPath(uri) {
    return this.normalizePath(import_coc35.Uri.parse(uri));
  }
  ...
  toResource(filepath) {
    if (this._apiVersion.gte(API.v213)) {
      if (filepath.startsWith(this.inMemoryResourcePrefix + "untitled:")) {
        let resource = import_coc35.Uri.parse(filepath);
  ...
  getDocument(resource) {
    if (resource.startsWith("untitled:")) {
      ...
      return import_coc35.workspace.getDocument(bufnr);
    }
    return import_coc35.workspace.getDocument(resource);
  }
  ...
  asUrl(filepath) {
    if (this._apiVersion.gte(API.v213)) {
      if (filepath.startsWith(this.inMemoryResourcePrefix + "untitled:")) {
        let resource = import_coc35.Uri.parse(filepath.slice(this.inMemoryResourcePrefix.length));
        if (this.inMemoryResourcePrefix) {
          const dirName = import_path8.default.dirname(resource.path);
          const fileName = import_path8.default.basename(resource.path);
          if (fileName.startsWith(this.inMemoryResourcePrefix)) {
    ...
    return import_coc35.Uri.file(filepath);
  }
  ...
  dispatchEvent(event) {
    switch (event.event) {
      ...
      case "projectsUpdatedInBackground":
        ...
        const resources = body.openFiles.map(import_coc35.Uri.file);
    ...
    if (this.apiVersion.gte(API.v230)) {
      ...
      pluginPaths = pluginPaths.reduce((p, c) => {
        if (import_path8.default.isAbsolute(c)) {
          ...
          let roots = import_coc35.workspace.workspaceFolders.map((o) => import_coc35.Uri.parse(o.uri).fsPath);
  ...
  getProjectRootPath(uri) {
    let root = import_coc35.workspace.cwd;
    let u = import_coc35.Uri.parse(uri);
    if (u.scheme !== "file")
      return void 0;
    let folder = import_coc35.workspace.getWorkspaceFolder(uri);
    if (folder) {
      root = import_coc35.Uri.parse(folder.uri).fsPath;
    } else {
      let filepath = import_coc35.Uri.parse(uri).fsPath;
  ...
  cancelInflightRequestsForResource(resource) {
    if (this.state !== import_coc35.ServiceStat.Running || !this.tsServerProcess) {
      return;
    }
...
var AtaProgressReporter = class {
  constructor(client) {
    ...
    this._invalid = false;
    this.statusItem = import_coc36.window.createStatusBarItem(10, { progress: true });
    const disposables = [];
  ...
  async onTypesInstallerInitializationFailed() {
    this.statusItem.hide();
    if (!this._invalid) {
      const config = import_coc36.workspace.getConfiguration("typescript");
      if (config.get("check.npmIsInstalled", true)) {
        const dontShowAgain = "Don't Show Again";
        const selected = await import_coc36.window.showWarningMessage("Could not install typings files for JavaScript language features. Please ensure that NPM is installed or configure 'typescript.npm' in your user settings. visit https://go.microsoft.com/fwlink/?linkid=847635  to learn more.", dontShowAgain);
...
var TypeScriptServiceClientHost = class {
  constructor(descriptions, pluginManager) {
    ...
    const configFileWatcher = import_coc37.workspace.createFileSystemWatcher("**/[tj]sconfig.json");
    ...
    const packageFileWatcher = import_coc37.workspace.createFileSystemWatcher("**/package.json");
    ...
    this.disposables.push(import_coc37.languages.registerWorkspaceSymbolProvider(new TypeScriptWorkspaceSymbolProvider(this.client, allModeIds)));
    this.client.onConfigDiagnosticsReceived((diag) => {
      let { body } = diag;
      if (body) {
        let { configFile, diagnostics } = body;
        let uri = import_coc37.Uri.file(configFile);
        if (diagnostics.length == 0) {
    ...
    import_coc37.workspace.onDidChangeConfiguration(this.configurationChanged, this, this.disposables);
    this.configurationChanged();
  }
  ...
  dispose() {
    (0, import_coc37.disposeAll)(this.disposables);
  ...
  async findLanguage(uri) {
    try {
      ...
      return languages4.find((language2) => language2.handlesUri(import_coc37.Uri.parse(uri)));
    } catch {
      return void 0;
    }
  }
...
var TsserverService = class {
  constructor(pluginManager) {
    ...
    this.id = "tsserver";
    this.name = "tsserver";
    this.state = import_coc38.ServiceStat.Initial;
  ...
  get config() {
    return import_coc38.workspace.getConfiguration("tsserver");
  }
  getClientHost() {
    if (this.state == import_coc38.ServiceStat.Running)
  ...
  start() {
    if (this.clientHost)
      return;
    this.state = import_coc38.ServiceStat.Starting;
...
var PluginManager = class {
  constructor() {
    ...
    import_coc39.extensions.onDidActiveExtension(loadPlugins, void 0, this._disposables);
    import_coc39.extensions.onDidUnloadExtension(loadPlugins, void 0, this._disposables);
  }
  dispose() {
    (0, import_coc39.disposeAll)(this._disposables);
  }
  get plugins() {
    if (!this._plugins) {
  ...
  readPlugins() {
    ...
    for (const extension of import_coc39.extensions.all) {
...
async function activate(context) {
  ...
  const config = import_coc40.workspace.getConfiguration().get("tsserver", {});
  if (!config.enable)
    return;
  ...
  service.start().then(() => {
    subscriptions.push(import_coc40.services.regist(service));
  }, (e) => {
